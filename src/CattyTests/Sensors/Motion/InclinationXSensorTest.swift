/**
 *  Copyright (C) 2010-2018 The Catrobat Team
 *  (http://developer.catrobat.org/credits)
 *
 *  This program is free software: you can redistribute it and/or modify
 *  it under the terms of the GNU Affero General Public License as
 *  published by the Free Software Foundation, either version 3 of the
 *  License, or (at your option) any later version.
 *
 *  An additional term exception under section 7 of the GNU Affero
 *  General Public License, version 3, is available at
 *  (http://developer.catrobat.org/license_additional_term)
 *
 *  This program is distributed in the hope that it will be useful,
 *  but WITHOUT ANY WARRANTY; without even the implied warranty of
 *  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 *  GNU Affero General Public License for more details.
 *
 *  You should have received a copy of the GNU Affero General Public License
 *  along with this program.  If not, see http://www.gnu.org/licenses/.
 */

import XCTest
import CoreMotion

@testable import Pocket_Code

final class InclinationXSensorTest: XCTestCase {

    var sensor : CBSensor = InclinationXSensor(motionManager: CMMotionManager())
    
    override func setUp() {
        guard sensor.isAvailable() else { return }
        
        sensor.start()
        XCTAssertTrue(sensor.isActive())
    }
    
    override func tearDown() {
        guard sensor.isAvailable() else { return }
        
        sensor.stop()
        XCTAssertFalse(sensor.isActive())
    }
    
    func testTransformToPocketCode() {
        XCTAssertEqual(0, sensor.transformToPocketCode(rawValue: 0))
        
        // TODO boundary tests
    }
    
    func testIsAvailableOnDevice() {
        guard UIDevice.isDevice else { return }
        
        XCTAssertTrue(sensor.isAvailable())
    }
    
    func testIsAvailableOnSimulator() {
        guard UIDevice.isSimulator else { return }
        
        XCTAssertFalse(sensor.isAvailable())
    }
    
    func testRawValue() {
        guard UIDevice.isDevice else { return }
        
        XCTAssertNotNil(sensor.rawValue())
    }
}
